...
- **三层漏斗检索**：语义匹配 → LLM 筛选 → 返回完整记忆，精准检索你的历史偏好
- **自我维护机制**：自动合并相似记忆、智能遗忘低价值记忆
- **情绪持久化**：信任🤝和好感❤️会跨会话保持，AI 记得你们的关系
- **结构化记忆**：每条记忆是 Skill 对象（标题/关键词/摘要/重要性），不是简单文本
- **成本最优**：日常检索只用轻量索引，必要时才加载全文
...
本项目实现了**类 Agent Skill 的智能记忆系统**，让 AI 真正"认识"你，而不是每次对话都从零开始：

- **记忆即 Skill**：每条记忆是结构化对象（标题/关键词/摘要/触发词/重要性），不是简单的文本存储
- **三层漏斗检索**：
  1. **第一层**：快速语义匹配（支持 Embedding 和 TF-IDF 回退）
  2. **第二层**：LLM 细粒度筛选真正相关的记忆
  3. **第三层**：返回完整记忆并更新访问记录
- **自我维护机制**：
  - 🔄 **自动合并**：相似记忆会被 LLM 智能合并，避免重复
  - 🕐 **智能遗忘**：低重要性、长期未访问的记忆会逐渐衰减归档
  - ⭐ **重要性评分**：记忆按 1-5 星评级，高价值记忆优先保留
- **情绪状态持久化**：
  - 信任🤝和好感❤️等慢通道情绪会持久化到数据库
  - 下次启动时恢复上次的情绪状态
  - AI 会记得你们之间的关系发展
- **成本最优设计**：
  - 日常检索只动用轻量索引，只在必要时加载原始全文
  - 无 Embedding 时自动回退到 TF-IDF 相似度计算
- **记忆示例**：
  ```
  [★★★★☆] 用户喜欢喝美式咖啡
     摘要：多次对话提到喜欢喝美式不加糖
